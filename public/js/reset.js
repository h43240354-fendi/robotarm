...
function setError(fieldId, errId, show, msg) {
  ...
  const err = $(errId);
  if (show) {
    inp && inp.classList.add('error-input');
    if (err) { if (msg) err.textContent = msg; err.classList.add('visible'); }
  } else {
    inp && inp.classList.remove('error-input');
...
makeToggle('togglePw',      'newPassword',     'eyeIcon');
...
$('newPassword').addEventListener('blur', () => {
  setError('password', 'err-password', $('newPassword').value.length < 8);
});
$('confirmPassword').addEventListener('blur', () => {
  setError('confirm', 'err-confirm', $('confirmPassword').value !== $('newPassword').value);
});
...
$('btnUpdate').addEventListener('click', async () => {
  ...
  if (otp.length !== 6) { setError('otp', 'err-otp', true, 'Please enter the 6-digit OTP code.'); valid = false; }
  ...
  $('btnUpdateText').textContent    = 'Updating...';
  ...
  try {
    ...
    if (!res.ok) throw new Error(text || 'Failed to update password.');

    clearInterval(countdownTimer);
    showToast('Password berhasil diperbarui', 'suc');
    ...
    $('btnUpdateText').textContent   = 'Update Password';
